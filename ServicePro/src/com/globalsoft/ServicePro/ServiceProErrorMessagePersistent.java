...
public class ServiceProErrorMessagePersistent {
    ...
    private static final String DATABASE_NAME = "OFFLINEDB2";
    ...
    public ServiceProErrorMessagePersistent(Context ctext){
    	context = ctext;    
    }
    ...
    public void closeDBHelper(){
    	try{
			...
			ServiceProConstants.showErrorLog("Error in ServiceProErrorMessagePersistent closeDBHelper:"+e.toString());
		}
    }//fn closeDBHelper       
    ...
    public ArrayList checkErrorTrancIdApiExists(){
    	...
    	try {
			DBHelper = new DatabaseHelper(context);
			sqlitedatabase = DBHelper.getWritableDatabase();
			c = sqlitedatabase.rawQuery("select * from "+DATABASE_TABLE+" where "+KEY_APINAME+"='"+ServiceProConstants.STATUS_UPDATE_API+"' or "+KEY_APINAME+"='"+ServiceProConstants.TASK_CONFIRMATION_API+"' or "+KEY_APINAME+"='"+ServiceProConstants.TASK_TRANS_API+"'", null);
			//ServiceProConstants.showLog("select * from "+DATABASE_TABLE+" where "+KEY_APINAME+"='"+ServiceProConstants.STATUS_UPDATE_API+"' or "+KEY_APINAME+"='"+ServiceProConstants.TASK_CONFIRMATION_API+"' or "+KEY_APINAME+"='"+ServiceProConstants.TASK_TRANS_API+"'");
			if (c.moveToFirst())
			{
			     do {
			...
			ServiceProConstants.showErrorLog("Error in checkErrorTrancIdApiExists:"+e.toString());
		}   
    ...
    public boolean checkTrancIdApiExists(String id, String apiName){
    	...
    	try {
			...
			ServiceProConstants.showErrorLog("Error in checkTrancIdApiExists:"+e.toString());
		}   
    	return idexists;
    ...
    {   	
    	...
    	try {
    		...
    		ServiceProConstants.showErrorLog("Error in getIdList:"+e.toString());
			sqlitedatabase.close();
		}
		return resArray;   
    }//fn getIdList 
    ...
    {   	
    	try {
			DBHelper = new DatabaseHelper(context);
			sqlitedatabase = DBHelper.getWritableDatabase();	
			sqlitedatabase.execSQL("DELETE FROM "+ServiceProDBAdapter.ERRLIST_TABLE+" WHERE "+KEY_TRANCID+"='"+tracId+"'");
			sqlitedatabase.close();
    	} catch (Exception e) {
    ...
    {   	
    	try {
			DBHelper = new DatabaseHelper(context);
			sqlitedatabase = DBHelper.getWritableDatabase();	
			sqlitedatabase.execSQL("DELETE FROM "+ServiceProDBAdapter.ERRLIST_TABLE+" WHERE "+KEY_TRANCID+"='"+tracId+"' and "+KEY_APINAME+"='"+apiName+"'");
			sqlitedatabase.close();
    	} catch (Exception e) {
    		ServiceProConstants.showErrorLog("Error in deleteRowForStatus:"+e.toString());
			sqlitedatabase.close();
		}   
    }//fn deleteRowForStatus
    
}//End of class ServiceProErrorMessagePersistent